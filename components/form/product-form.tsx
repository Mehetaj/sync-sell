...
type Action =
  | { type: "SET_FIELD"; field: keyof IProduct; value: string }
  | { type: "SET_SIZES"; value: string[] }
  | { type: "SET_MAIN_IMAGE"; value: string }
  | { type: "ADD_ADDITIONAL_IMAGE"; value: string }
  | { type: "REMOVE_ADDITIONAL_IMAGE"; index: number }
  | { type: "RESET_FORM" }
...
function productReducer(state: IProduct, action: Action): IProduct {
  switch (action.type) {
    case "SET_FIELD":
...
export function ProductForm() {
  ...
  const { items, loading: categoriesLoading } = useSelector((state: RootState) => state.catalog)
  ...
  const handleInputChange = (e: React.ChangeEvent<HTMLInputElement | HTMLSelectElement | HTMLTextAreaElement>) => {
    formDispatch({ type: "SET_FIELD", field: e.target.name as keyof IProduct, value: e.target.value })
  }
  ...
  return (
    <form onSubmit={handleSubmit} className="mx-auto p-6 font-metal">
      ...
      <motion.button
        ...
        type="submit"
        disabled={isLoading}
        className="w-full bg-black text-white py-3 font-medium tracking-wider hover:bg-gray-800 transition-colors disabled:opacity-50"
      >
